...
function App() {
    ...
    const handleUpdateImportedList = (newList) => {
        setList((prevList) => ({
            data: [
                ...prevList.data.concat(
                    ...newList.filter(
                        (newData) =>
                            !!newData.url &&
                            !prevList.data.some(
                                (prevItem) => prevItem.url === newData.url
                            )
                    )
                ),
            ],
        ...
        <div style={AppStyles.app()}>
            {pageState === 1 && (
                <List
                    ...
                    updateImportedList={handleUpdateImportedList}
                />
            )}
            {pageState === 2 && (
                <Details
